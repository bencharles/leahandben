...
def main():
    ...
    # 3. Assemble FAQ (with auth check)
    faq_html = assemble('faq.html', auth_check=auth)
    faq_out = ROOT / 'faq.html'
    faq_out.write_text(faq_html)
    print(f'  faq.html                ({len(faq_html)/1024:.1f} KB)')

    # 4. Assemble Travel (with auth check)
